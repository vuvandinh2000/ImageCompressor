...
#compressed_file: file name with path & extension
#decompressed_image: Path to file for decompressed image, default='./'
#iden: Y/y for decompressed jpg
def decompressor(compressed_file, decompressed_image='./', iden='y'):
    _, tail = os.path.split(compressed_file)
    if iden == 'Y' or iden == 'y': image_save = decompressed_image + os.path.splitext(tail)[0] + '.jpg'
    ...
    for _ in pbar: comp = EntropyReduction.bz2_unc(compressed_file)
